...
import 'package:tcord/views/profile/settingsPopup.dart';
...
class MyProfilePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    MyProfilePageController controller =
        Get.put(MyProfilePageController(context));
    return Scaffold(
...
class HeaderWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    MyProfilePageController controller = Get.find();
    return Obx(() => Padding(
          padding:
                  ...
                  Row(
                      ...
                      children: [
                        ...
                        Padding(
                          padding: const EdgeInsets.only(top: 0, left: 17),
                          child: InkWell(
                            onTap: () async => await controller.openFollowing(),
                            child: Column(
                              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                              crossAxisAlignment: CrossAxisAlignment.center,
                              children: [
                ...
                child: Center(
                  child: Row(
                    ...
                    children: [
                      InkWell(
                        onTap: () async {
                          Get.to(() => EditProfilePage(controller.refresh));
                        },
                        child: Container(
                            width: 270,
                            height: 35,
                            ...
                            decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                  width: 0.5,
                                  color: AppTheme.GrayWeb.withOpacity(0.5),
                                ),
...
class MyProfilePageController extends BaseGetxController {
  ...
  MyProfilePageController(BuildContext context) : super(context);

  Future refresh() async {
    isBusy.value = true;
    var response = await _userService
        .getUser(authenticationService.authenticationData!.user!.id!);

    user.value = response;
    var comps;
    if (isCompositionsPageSelected.value) {
      comps = await _userService.getUserCompositions(
          authenticationService.authenticationData!.user!.id!);
    } else if (isDebutsPageSelected.value) {
      comps = await _userService
          .getUserDebuts(authenticationService.authenticationData!.user!.id!);
    } else if (isLikesPageSelected.value) {
      comps = await _userService
          .getUserLikes(authenticationService.authenticationData!.user!.id!);
    }
  ...
  Future openFollowers() async {
    isBusy.value = true;
    var response = await _userService
        .getUserFollowers(authenticationService.authenticationData!.user!.id!);
    await showPopup(
  ...
  Future openFollowing() async {
    isBusy.value = true;
    var response = await _userService
        .getUserFollowing(authenticationService.authenticationData!.user!.id!);
    await showPopup(
